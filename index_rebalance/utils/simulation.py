...
def get_next_ftse_review_date(input_date):
    """
    Given a date string in the format '%Y-%m-%d', return the next upcoming 
    FTSE UK Index Series review cutoff date as a string in the same format.
...
def gbm(
    ...
    """
    Vectorized Geometric Brownian Motion (GBM) for multiple stocks with lists of parameters.
...
def calculate_position(df: pd.DataFrame):
    """Compute and rank tickers by estimated market cap, add the following additional columns:
        - number of shares
...
def apply_ftse100_review_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    3. A constant number of constituents will be maintained for the FTSE 100. Where a greater number of companies qualify to be inserted in an index than those qualifying to be deleted, the lowest-ranking constituents presently included in the index will be deleted to ensure that an equal number of companies are inserted and deleted at the periodic review. Likewise, where a greater number of companies qualify to be deleted than those qualifying to be inserted, the securities of the highest-ranking companies that are presently not included in the index will be inserted to match the number of companies being deleted at the periodic review.
    Based on the rules, add the following additional columns:
    - potential insert: True if the company is not currently an constituent and rises above 90th, otherwise False
    - potential delete: True if the company is currently an constituent and falls below 111th, otherwise False
    - forced delete: True if the company is one of the lowest-ranking constituents presently included to be deleted when the number of potential insert exceeds the number of potential delete
    - forced insert: True if the company is one of the highest-ranking non-constituents presently included to be inserted when the number of potential delete exceeds the number of potential insert
    - revision decision: the value could be one of ['stay', 'added', 'removed', 'not included']

    Args:
        df (pd.DataFrame): DataFrame which column 'estimated market cap rank' indicates its position.
    """
    df = df.sort_values("estimated market cap rank").reset_index(drop=True)

    # Step 1: flag potential adds and deletes
    df["potential insert"] = (df["constituent"] == 0) & (df["estimated market cap rank"] <= 90)
    df["potential delete"] = (df["constituent"] == 1) & (df["estimated market cap rank"] >= 111)
    ...
    assert len(df[df['review decision'] == 'added']) == len(df[df['review decision'] == 'removed']), f"{len(df[df['review decision'] == 'added'])} addition and {len(df[df['review decision'] == 'removed'])} remove"
    return df

def single_simulation(df: pd.DataFrame, rank_date: str):
    """Run a single simulation based on Geometric Brownian Motion (GBM)
    Step 1 - Run GBM to estimate the share price at rank date considering volatility & drift
...
def multiple_simulation(df: pd.DataFrame, rank_date: str, n: int):
    ...
    columns_to_copy = ["ticker", "company", "price", "market cap", "volatility", "drift"]
    ...
    # assert simulation_df['added'].sum() == simulation_df['removed'].sum(), f'Unequal number of addition and delete'
