...
from index_rebalance.utils.simulation import get_next_ftse_review_date, multiple_simulation
...
def main():
    ...
    if 'market_data' not in st.session_state:
        st.session_state.market_data = copy.deepcopy(st.session_state.fetched_market_data)
    ...
    st.session_state.task = st.sidebar.selectbox(
        ...
        index=task_options.index(st.session_state.task) if st.session_state.task else None,
    ...
    if st.session_state.task is None:
        ...
        st.write(f'Addition Threshold: {st.session_state.market_data.iloc[89]["market cap"]}')
        st.write(f'Deletion Threshold: {st.session_state.market_data.iloc[109]["market cap"]}')
        def reset_market_data():
            st.session_state.market_data = copy.deepcopy(st.session_state.fetched_market_data)
        st.button(
            label='reset',
            on_click=reset_market_data
        )
        # st.dataframe(
